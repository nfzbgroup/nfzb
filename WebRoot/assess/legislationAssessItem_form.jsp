...
<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li>
            <span ><c:if test="${nodeId=='NOD_0000000252'}"><c:choose><c:when test="${legislationAssessTask.stTaskId !=null}"><c:if test="${legislationAssessTask.stTaskStatus=='TODO'}">编辑</c:if><c:if test="${legislationAssessTask.stTaskStatus=='DONE'}">详情</c:if></c:when><c:otherwise>发起</c:otherwise></c:choose> </c:if><c:if test="${nodeId!='NOD_0000000252'}">详情</c:if></span>
        </li>
    </ul>
	...
	<form id="legislationAssessItemForm" class="form-horizontal"
        ...
        <div class="form-body">
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <input type="text" class="form-control" name="stItemName" <c:if test="${(nodeId!='NOD_0000000252')||(legislationAssessTask.stTaskStatus !=null&&legislationAssessTask.stTaskStatus !='TODO')}">disabled</c:if> <c:if test="${legislationAssessItem.stItemName !=null}">value="${legislationAssessItem.stItemName}" </c:if>>
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-3 control-label">评估规划：</label>
                <div class="col-sm-9">
                    <select class="form-control" name="stAssessId" <c:if test="${(nodeId!='NOD_0000000252')||(legislationAssessTask.stTaskStatus !=null&&legislationAssessTask.stTaskStatus !='TODO')}">disabled</c:if>>
                       <c:if test="${legislationAssessList!=null&&fn:length(legislationAssessList)>0}">
                           <c:forEach var="assess" items="${legislationAssessList}">
                               <option value="${assess.stAssessId}" <c:if test="${legislationAssessItem.stAssessId !=null&&legislationAssessItem.stAssessId ==assess.stAssessId}">selected</c:if>>${assess.stAssessName}</option>
                           </c:forEach>
                       </c:if>
                    </select>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <textarea class="form-control" name="stBak" <c:if test="${(nodeId!='NOD_0000000252'&&nodeId!='NOD_0000000207')||(legislationAssessTask.stTaskStatus !=null&&legislationAssessTask.stTaskStatus !='TODO')}">disabled</c:if> ><c:if test="${legislationAssessItem.stBak !=null}">${legislationAssessItem.stBak}</c:if></textarea>
                </div>
            </div>
			<div class="form-group text-center">
                <c:if test="${nodeId=='NOD_0000000252'&&(legislationAssessTask.stTaskStatus ==null||legislationAssessTask.stTaskStatus=='TODO')}">
                    <input type="button" class="btn btn-w-m btn-success"  value="保存" onclick="saveLegislationAssessItem()"> &nbsp;&nbsp;
                </c:if>
					<input type="button" class="btn btn-w-m btn-success" data-dismiss="modal" value="关闭">
			</div>
            <div class="form-group">
                <label class="control-label">评估项目材料
                </label>
                <c:if test="${nodeId=='NOD_0000000252'&&(legislationAssessTask.stTaskStatus ==null||legislationAssessTask.stTaskStatus=='TODO')}">
...
<script>
    function saveLegislationAssessItem() {
        ...
        if(param.stItemName==null||param.stItemName==""){
            Duang.error("提示","请输入评估项目名称");
        }else if(param.stAssessId==null||param.stAssessId==""){
            Duang.error("提示","请选择评估规划");
        }else {
