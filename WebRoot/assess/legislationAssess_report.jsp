...
<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li>
            <span ><c:choose><c:when test="${nodeId=='NOD_0000000262'&&(legislationAssessTask.stTaskStatus ==null||legislationAssessTask.stTaskStatus=='TODO')}">录入</c:when><c:otherwise>查看</c:otherwise></c:choose></span>
        </li>
    </ul>
...
<div class="modal-body">
	<form id="legislationAssessForm" class="form-horizontal"
        ...
        <div class="form-body">
            ...
            <div class="form-group">
                <label class="col-sm-3 control-label">情况报告内容：</label>
                <div class="col-sm-9">
                    <textarea class="form-control" name="stComment1" <c:if test="${nodeId !='NOD_0000000262'||(legislationAssessTask.stTaskStatus !=null&&legislationAssessTask.stTaskStatus=='DONE')}">disabled</c:if> ><c:if test="${legislationAssessTask.stComment1 !=null}">${legislationAssessTask.stComment1}</c:if></textarea>
                </div>
            </div>
			<div class="form-group text-center">
                <c:if test="${nodeId=='NOD_0000000262'&&(legislationAssessTask.stTaskStatus ==null||legislationAssessTask.stTaskStatus=='TODO')}">
                    <input type="button" class="btn btn-w-m btn-success"  value="保存" onclick="saveLegislationAssess()"> &nbsp;&nbsp;
                </c:if>
					<input type="button" class="btn btn-w-m btn-success" data-dismiss="modal" value="关闭">
			</div>
            <div class="form-group">
                <label class="control-label">情况报告材料
                </label>
                <c:if test="${nodeId=='NOD_0000000262'&&(legislationAssessTask.stTaskStatus ==null||legislationAssessTask.stTaskStatus=='TODO')}">
