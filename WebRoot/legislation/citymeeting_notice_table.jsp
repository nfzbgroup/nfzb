						...
						<tr class="text-center">
							...
							<td ><fmt:formatDate type="date" value="${task.dtSendDate}" /></td>
									<c:choose>
										...
										<c:otherwise>
											<td ><a href="javascript:void(0);" onclick="openCitymeetingPage('city_meeting__TODO','${task.stNoticeId}')" class="layer_full_link">查看</a></td>
										</c:otherwise>
									</c:choose>
						</tr>
					</c:forEach>
...
<script>
    ...
    function uploadNotice(stDocId, stNodeId,stNoticeId) {
		$.post("../legislationProcessTask/uploadReport.do?stDocId=" + stDocId + "&stNode=" + stNodeId, function(data) {
			if (data.success) {
				//提交操作，到达下一个节点
				nextChildProcessNotice(stNoticeId);
			} else {
				Duang.error("提示", "请补全必填材料！");
			}
		}, "json")
	};
	function nextChildProcessNotice(stNoticeId) {
		layer.confirm('请确认操作！', function(index) {
			layer.close(layer.index);
			$.post("../legislationProcessTask/draft_task_list.do?stNoticeId="+stNoticeId+"&method=nextChildProcess");
			 submitForm(1);
		});
	};
    function nextProcess(stTopicId,stNoticeId,method) {
        layer.confirm('请确认操作！',function(index){
            layer.close(layer.index);
			$.post("../legislationProcessTask/draft_task_list.do?stTopicId="+stTopicId+"&stNoticeId="+stNoticeId+"&method="+method);
            submitForm(1);
        });
    }
