...
<div class = "conteng">
    ...
    <div class = "conteng-title">
        <span>当前位置：</span><em>立法后评估报告列表-首页</em>
    </div>
	<form name="form1" class = "form-horizontal" action="${basePath}/legislate/assessment_report_list.do" method="post" role = "form">   
          <div class="col">
	         <div class="control-group">
                 ...
                 <div >
                    <s:select list="#{'assessmentTitle':'报告名称','agentUnitName':'上报单位'}" name="queryType" cssClass="input-xlarge"></s:select>
                 </div>
                 <label  class="control-label">
    ...
    <div class = "detai">
        <div class = "detai-con">
            <div id="init" class = "tab-list <s:if test='#request.receive == "N"'>tab-back</s:if> " onclick="window.location.href='${basePath}/legislate/assessment_report_list.do?receive=N'">未接收</div>
            <div id="send" class = "tab-list <s:if test='#request.receive == "Y"'>tab-back</s:if>"  onclick="window.location.href='${basePath}/legislate/assessment_report_list.do?receive=Y'">已接收</div>
        <div class = "biaodan">
	                 	...
	                 	<tr>
	                        ...
	                        <td>
	                           ...
	                           <s:if test="#request.currentTeam.id=='U_3_3'">
	                      	     ...
	                      	     <a href="${basePath }/legislate/assessment_report_dis_unit.do?assessmentId=<s:property value="#info.assessmentId" />" class = "tianbao caozuo-btn" >分送</a>
	                      	   </s:if>
	                      	   <s:else>
	                      	   	  <a class = "tianbao caozuo-btn layer-fenban" onclick="assessmentModify('<s:property value="#info.assessmentId" />')">修改</a>
	                      	      <a class = "tianbao caozuo-btn" onclick="assessmentUpReport('<s:property value="#info.assessmentId" />')">上报</a>
	                      	   </s:else>
	                        </td>
	                    </tr>
	                   </s:iterator>
...
var uuu;
    ...
    function assessmentDetail(assessmentId){
    	window.location.href = '${basePath}/legislate/assessment_report_detail.do?assessmentId='+assessmentId;
    }
    function assessmentModify(assessmentId){
    	window.location.href = '${basePath}/legislate/assessment_report_add.do?assessmentId='+assessmentId;
    }
    ...
    function assessmentReportDis(assessmentId,unitId){
    	$.ajax({
    		url:'${basePath}/legislate/assessment_report_dis.do',
    ...
    function assessmentReportBack(assessmentId,unitId){
    	$.ajax({
    		url:'${basePath}/legislate/assessmentReportBack.do',
    		data:'assessmentId='+assessmentId,
    		type:'post',
    		async:false,
    		success:function(msg){
