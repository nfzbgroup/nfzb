...
<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li>
            <span ><c:choose><c:when test="${stTaskId !=null}"><c:if test="${legislationPlanTask.stTaskStatus=='TODO'}">编辑</c:if><c:if test="${legislationPlanTask.stTaskStatus=='DONE'}">详情</c:if></c:when><c:otherwise>发起</c:otherwise></c:choose> </span>
        </li>
    </ul>
...
<div class="modal-body">
	<form id="legislationPlanForm" class="form-horizontal"
        ...
        <input hidden name="stTaskId" <c:if test="${stTaskId !=null}">value="${stTaskId}" </c:if>>
        <div class="form-body">
            <div class="form-group">
                <label class="col-sm-3 control-label">立法项目名称：</label>
                <div class="col-sm-9">
                    <input type="text" class="form-control" name="stPlanName" <c:if test="${legislationPlanTask.stTaskStatus !=null&&legislationPlanTask.stTaskStatus=='DONE'}">readonly</c:if> <c:if test="${legislationPlan.stPlanName !=null}">value="${legislationPlan.stPlanName}" </c:if>>
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-3 control-label">立法项目类型：</label>
                <div class="col-sm-9">
                    <select class="form-control" name="stPlanType" <c:if test="${legislationPlanTask.stTaskStatus !=null&&legislationPlanTask.stTaskStatus=='DONE'}">readonly</c:if>>
                        <option value="立">立</option>
                    </select>
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <textarea class="form-control" name="stReason" <c:if test="${legislationPlanTask.stTaskStatus !=null&&legislationPlanTask.stTaskStatus=='DONE'}">readonly</c:if> ><c:if test="${legislationPlan.stReason !=null}">${legislationPlan.stReason}</c:if></textarea>
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-3 control-label">目前进展情况：</label>
                <div class="col-sm-9">
                    <textarea class="form-control" name="stProgress" <c:if test="${legislationPlanTask.stTaskStatus !=null&&legislationPlanTask.stTaskStatus=='DONE'}">readonly</c:if> ><c:if test="${legislationPlan.stProgress !=null}">${legislationPlan.stProgress}</c:if></textarea>
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-3 control-label">备注：</label>
                <div class="col-sm-9">
                    <textarea class="form-control" name="stRemark" <c:if test="${legislationPlanTask.stTaskStatus !=null&&legislationPlanTask.stTaskStatus=='DONE'}">readonly</c:if> ><c:if test="${legislationPlan.stRemark !=null}">${legislationPlan.stRemark}</c:if></textarea>
                </div>
            </div>
...
<script>
    function saveLegislationPlan() {
        ...
        }else if(param.stPlanType==null||param.stPlanType==""){
            Duang.error("提示","请选择立法项目类型");
        }else if(param.stReason==null||param.stReason==""){
            Duang.error("提示","请输入建议立项理由");
        }else if(param.stProgress==null||param.stProgress==""){
